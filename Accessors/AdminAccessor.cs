...
namespace RadialReview.Accessors
{
	...
	public class AdminAccessor : BaseAccessor
	{
		...
		private static CircularQueue<LatestExport> AllUserDataExports = new CircularQueue<LatestExport>(4);
		[Obsolete("Not safe and expensive")]
		public static CircularQueue<LatestExport> GetExportList()
		{
			...
			using (var s = HibernateSession.GetCurrentSession())
			{
				using (var tx = s.BeginTransaction())
				{
					...
					var allOrgsF = s.QueryOver<OrganizationModel>()
						...
						.Select(x => x.Id, x => x.Name, x => x.DeleteTime, x => x.CreationTime, x => x.AccountType, x => paymentPlanAlias.FreeUntil, x => x._Settings.EnableL10, x => x._Settings.EnablePeople, x => x._Settings.EnableReview)
						.Future<object[]>();
					...
					.Left.JoinAlias(x => x.Organization, () => orgAlias).Where(x => x.DeleteTime != null || orgAlias.DeleteTime != null).Select(x => x.Id, x => x.DeleteTime, x => orgAlias.DeleteTime).Future<object[]>().Select(x => new {
						Id = (long)x[0],
						DeleteTime = ((DateTime?)x[1] ?? (DateTime?)x[2]),
					});
					var meetingsByCompanyF = s.QueryOver<L10Meeting>().Where(x => x.CompleteTime != null && x.Preview == false).Select(x => x.OrganizationId, x => x.StartTime, x => x.CompleteTime).Future<object[]>().Select(x => new {
						OrgId = (long)x[0],
						StartTime = (DateTime?)x[1],
						CompleteTime = (DateTime?)x[2],
					});
					var paymentTokens = s.QueryOver<PaymentSpringsToken>().Where(x => x.Active == true && x.DeleteTime == null).Select(x => x.OrganizationId, x => x.MonthExpire, x => x.YearExpire, x => x.TokenType, x => x.Address_1, x => x.Address_2, x => x.City, x => x.Country, x => x.Phone, x => x.State, x => x.Zip).Future<object[]>().Select(x => new {
						OrgId = (long)x[0],
						MonthExpire = (int)x[1],
						YearExpire = (int)x[2],
					...
					var allUserNames = s.QueryOver<UserModel>().Select(x => x.UserName, x => x.FirstName, x => x.LastName, x => x.DeleteTime).Future<object[]>().Select(x => new {
						Email = (string)x[0],
						FN = (string)x[1],
						LN = (string)x[2],
					...
					var nodesF = s.QueryOver<AccountabilityNode>().Where(x => x.DeleteTime == null).Select(x => x.Id, x => x.ParentNodeId //x => x.DeprecatedUserId
					).Future<object[]>();
					...
					var charts = chartsF.Select(x => new {
						RootId = x
					}).ToList();
					...
					{
						if (o.UserEmail.ToLower().EndsWith("@mytractiontools.com") || o.UserEmail.ToLower().EndsWith("@bloomgrowth.com") || o.UserEmail.ToLower().EndsWith("@winterinternational.io"))
						{
							continue;
						}
					...
					AdminAccessor.AllUserDataExports.Enqueue(new LatestExport() { Id = ExportId++, Data = csv, GeneratedAt = now, GenerationDuration = elapsed });
					return csv;
				}
			}
		}
		...
		public static string LogError(HttpContext context, Exception ex)
		{
			var name = context.NotNull(x => x.User.Identity.Name);
			var httpMethod = context.NotNull(x => x.Request.Method);
			var url = context.NotNull(x => x.Request.Path.NotNull(y => new Uri(y)));
			var referrer = context.NotNull(x => x.Request.Headers[HeaderNames.Referer].NotNull(y => new Uri(y)));
		...
		protected static ServerSettings<bool> ReadonlySettings = new ServerSettings<bool>(s => s.GetSettingOrDefault(Variable.Names.READ_ONLY_MODE, false), TimeSpan.FromMinutes(.25));
		public static string LogError(string userId, string httpMethod, Uri url, Uri referrer, Exception ex)
		{
			try
			{
				log.Error(string.Format("[LogError] {0} {1} {2} {3} {4} [{5}] [{6}] [{7}]", /*{0}*/ userId, /*{1}*/ httpMethod, /*{2}*/ url.NotNull(x => x.AbsolutePath), /*{3}*/ url.NotNull(x => x.Query), /*{4}*/ referrer.NotNull(x => x.AbsolutePath), /*{5}*/ ex.NotNull(x => x.GetType().Name), /*{6}*/ ex.NotNull(x => x.Message), /*{7}*/ ex.NotNull(x => x.StackTrace)));
			}
			catch (Exception e)
			{
			...
			try
			{
				using (var s = HibernateSession.GetCurrentSession())
				{
					using (var tx = s.BeginTransaction())
					{
						var err = new ErrorLog() { Message = ex.NotNull(x => x.Message), StackTrace = ex.NotNull(x => x.StackTrace), ExceptionType = ex.NotNull(x => x.GetType().Name), Path = url.NotNull(x => x.AbsolutePath), QueryParams = url.NotNull(x => x.Query), HttpMethod = httpMethod, UserId = userId };
						if (!ReadonlyMode)
						{
		...
		public static List<AdminAccessModel> GetAdminAccessLogs_Unsafe(int days)
		{
			using (var s = HibernateSession.GetCurrentSession())
			{
				using (var tx = s.BeginTransaction())
				{
					...
					var adminQ = s.QueryOver<UserModel>().WhereRestrictionOn(x => x.Id).IsIn(adminsIds).Select(x => x.FirstName, x => x.LastName, x => x.UserName, x => x.Id).Future<object[]>().Select(x => new {
						firstName = (string)x[0],
						lastName = (string)x[1],
						email = (string)x[2],
						Id = (string)x[3]
					});
					var userEmailQ = s.QueryOver<UserModel>().WhereRestrictionOn(x => x.UserName).IsIn(userEmails).Select(x => x.FirstName, x => x.LastName, x => x.UserName).Future<object[]>().Select(x => new {
						firstName = (string)x[0],
						lastName = (string)x[1],
						email = (string)x[2]
					});
					...
					var accessQ = s.QueryOver<UserOrganizationModel>()
						...
						.Select(x => userAlias.FirstName, x => userAlias.LastName, x => userAlias.UserName, x => x.Id, x => orgAlias.Name).Future<object[]>()
						.Select(x => new {
							firstName = (string)x[0],
							lastName = (string)x[1],
							email = (string)x[2],

...
namespace RadialReview.Accessors {
  public class ApplicationAccessor : BaseAccessor {
    ...
    public static async Task<bool> EnsureApplicationExists(UserManager<UserModel> userManager) {
      if (Config.GetEnv() == Env.production && Config.GetAppSetting("IsAwsConfig", "False") != "True") {
        throw new Exception("Ensure you've published with the AWS configuration transform.");
      }
      {
        using (var s1 = HibernateSession.GetCurrentSession()) {
          if (await HibernateSession.Unsafe.ShouldUpdateDBCache()) {

            using (var tx = s1.BeginTransaction()) {
              ConstructPositions(s1);
              tx.Commit();
              s1.Flush();
            }
            List<QuestionCategoryModel> applicationCategories;
            using (var tx = s1.BeginTransaction()) {
              applicationCategories = ConstructApplicationCategories(s1);
              tx.Commit();
              s1.Flush();
            }
            using (var tx = s1.BeginTransaction()) {
              ConstructApplicationQuestions(s1, applicationCategories);
              tx.Commit();
              s1.Flush();
            }

            using (var tx = s1.BeginTransaction()) {
              ConstructApplicationTasks(s1);
              tx.Commit();
              s1.Flush();

            }
            using (var tx = s1.BeginTransaction()) {
              ConstructPhoneNumbers(s1);
              tx.Commit();
              s1.Flush();
            }
          }
          ...
          //always execute..
          using (var tx = s1.BeginTransaction()) {
            var lockCount = s1.QueryOver<SyncLock>().WhereRestrictionOn(x => x.Id).IsLike("__CreateKey_%").RowCount();
            SyncLock.MAX_SYNC_KEYS = lockCount;
            s1.UpdateSetting(Variable.Names.MAX_SYNC_KEYS, lockCount);
            tx.Commit();
            s1.Flush();
          }
        }
			...
			using (var s = HibernateSession.GetDatabaseSessionFactory().OpenStatelessSession()) {
				foreach (var kgroup in Enumerable.Range(0, SyncLock.MAX_SYNC_KEYS).GroupBy(x => (int)(x / 50))) {
					var keys = kgroup.Select(x => SyncLock.CREATE_KEY(x)).ToArray();
					using (var tx = s.BeginTransaction(IsolationLevel.Serializable)) {
						var found = s.QueryOver<SyncLock>().WhereRestrictionOn(x => x.Id).IsIn(keys).Select(x => x.Id).List<string>();
						var any = false;
						foreach (var k in keys.Where(x => !found.Any(y => y == x)).ToList()) {
							s.Insert(new SyncLock() { Id = k });
							any = true;
						}
						if (any) {
							tx.Commit();
			...
			log.Info("Executed create synclocks:" + totalMs + "ms");
#endif

      await ConstructDefaultAdminOrgsForLocalhost(userManager);
    ...
    public static async Task ConstructDefaultAdminOrgsForLocalhost(UserManager<UserModel> userManager) {
      var name = "admin@tt.com";
      if (Config.IsLocal()) {
        var shouldRun = false;
        using (var s = HibernateSession.GetCurrentSession()) {
          using (var tx = s.BeginTransaction()) {
            if (s.QueryOver<UserModel>().Where(x => x.UserName == name).RowCount() == 0) {
              shouldRun = true;
            }
          }
        }
        if (shouldRun) {
          ...
          //var UserManager = new NHibernateUserManager();
          await UserAccessor.CreateUser(userManager, user, Guid.NewGuid().ToString() + "A!");
          using (var s = HibernateSession.GetCurrentSession()) {
            var pp = PaymentAccessor.GetPlanType("professional");
            var orgCreationData = new OrgCreationData() {
            ...
            using (var userCacheUpdater = new UserCacheUpdater()) {
              var res = await OrganizationAccessor.CreateOrganization(s, user, pp, DateTime.UtcNow, orgCreationData, false, userCacheUpdater);
              using (var tx = s.BeginTransaction()) {
                var u = s.Get<UserOrganizationModel>(res.NewUser.Id);
                u.IsRadialAdmin = true;
                s.Update(u);
                tx.Commit();
                s.Flush();
              }
    ...
    public const string CLOSE_LONG_MEETINGS = "CLOSE_LONG_MEETINGS";
    ...
    public static void ConstructApplicationTasks(ISession s) {
      ...
      var closeLongMeetings = s.QueryOver<ScheduledTask>().Where(x => x.DeleteTime == null && x.Executed == null && x.TaskName == CLOSE_LONG_MEETINGS).List().ToList();
      if (!closeLongMeetings.Any()) {
        var time = DateTime.UtcNow.AddMinutes(-60);
        var b = new DateTime(time.Year, time.Month, time.Day, time.Hour, 15, 0);
        var task = new ScheduledTask() {
          MaxException = 10000000,
          Url = "/Scheduler/CloseMeetings",
          NextSchedule = TimeSpan.FromHours(1),
          Fire = b,
          FirstFire = b,
          TaskName = CLOSE_LONG_MEETINGS,
    ...
    public static QuestionCategoryModel GetRockCategory(ISession s) {
      return GetApplicationCategory(s, EVALUATION);
    }

	...
	public class CustomizeAccessor {
		public static List<SelectListItem> AllProperties() {
			return new List<SelectListItem>(){
				...
				new SelectListItem(){Text = "Check-In Subheading", Value = CUSTOMIZABLE.Segue_Subheading},
		...
		public static List<CustomText> GetCustomizations(UserOrganizationModel caller, long organizationId) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					return s.QueryOver<CustomText>().Where(x => x.DeleteTime == null && x.OrgId == organizationId).OrderBy(x => x.CreateTime).Desc.List().GroupBy(x => x.PropertyName).Select(x => x.First()).ToList();
				}
			}
		}
		...
		public static string GetSpecificCustomization(UserOrganizationModel caller, long orgId, string propertyName, string dflt = null) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					var found = s.QueryOver<CustomText>().Where(x => x.DeleteTime == null && x.OrgId == orgId && x.PropertyName == propertyName).OrderBy(x => x.CreateTime).Desc.Take(1).SingleOrDefault().NotNull(x => x.NewText);

...
namespace RadialReview.Accessors {
	public class InvoiceAccessor {
		...
		public static List<InvoiceModel> AllOutstanding_Unsafe(UserOrganizationModel caller) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					var perms = PermissionsUtility.Create(s, caller);
					perms.RadialAdmin();
		...
		public static InvoiceModel GetInvoice(UserOrganizationModel caller, long invoiceId) {
			...
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					invoice._InvoiceUserItems= s.QueryOver<InvoiceUserItemModel>().Where(x => x.InvoiceId == invoiceId && x.DeleteTime == null).List().ToList();

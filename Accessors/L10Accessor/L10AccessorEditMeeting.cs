...
namespace RadialReview.Accessors {
  public partial class L10Accessor : BaseAccessor {
        ...
        using (var tx = s.BeginTransaction()) {
          ...
          if (oldMeeting != null) {
            ...
            if (!updateRocks.AddedValues.All(x => updatedRocks.Any(y => y.Id == x.Id))) {
              throw new PermissionsException("You do not have access to add one or more goal.");
            }
          }
          ...
          /////////////
          //Update goals on the VTO also

          //we need to make sure to set this or the goals is duplicated.
          ...
          await Audit.L10Log(s, caller, l10Recurrence.Id, "EditL10Recurrence", ForModel.Create(l10Recurrence));
    ...
    public static async Task<string> UpdatePage(UserOrganizationModel caller, long forUserId, long recurrenceId, string pageName, string connection, DateTime? startTime = null) {
          ...
          if (addNew) {
            ...
            if (meeting.MeetingLeader.NotNull(x => x.Id) == forUserId) {
              ...
              await using (var rt = RealTimeUtility.Create()) {
                var meetingHub = rt.UpdateGroup(RealTimeHub.Keys.GenerateMeetingGroupId(meeting));
                var baseMins = meeting._MeetingLeaderPageDurations.SingleOrDefault(x => x.Item1 == pageName).NotNull(x => x.Item2);
                var now_ms = now.ToJavascriptMilliseconds();

                meetingHub.Call("setCurrentPage", pageName.ToLower(), now_ms, baseMins/*, pageType*/);
                meetingHub.Update(new AngularMeeting(recurrenceId) { CurrentPage = pageName });
                await HooksRegistry.Each<IMeetingEvents>((ses, x) => x.CurrentPageChanged(ses, recurrenceId, meeting, pageName.ToLower(), now_ms, baseMins));


                foreach (var a in meeting._MeetingLeaderPageDurations) {
                  if (a.Item1 != pageName) {
                    meetingHub.Call("setPageTime", a.Item1, a.Item2);
                  }
                }
              }
            }
...
#pragma warning restore CS0618 // Type or member is obsolete
          ...
          await Audit.L10Log(s, caller, recurrenceId, "UpdatePage", ForModel.Create(meeting), friendlyPageName);
          tx.Commit();
          s.Flush();
          return pageType;
        }
    ...
    public static Ratio GetAverageMeetingRatingMostRecent(UserOrganizationModel caller, long recurrenceId) {
      using (var s = HibernateSession.GetCurrentSession()) {
        using (var tx = s.BeginTransaction()) {
          var perms = PermissionsUtility.Create(s, caller);
          perms.ViewL10Recurrence(recurrenceId);
          var current = _GetCurrentOrMostRecentL10Meeting(s, perms, recurrenceId, true, false, false);
          if (current == null || current.AverageMeetingRating == null)
            return new Ratio(0, 0);
          return current.AverageMeetingRating;
        }
      }
    }
    ...
    public static async Task DeleteL10Recurrence(UserOrganizationModel caller, long recurrenceId) {
      ...
      using (var s = HibernateSession.GetCurrentSession()) {
        using (var tx = s.BeginTransaction()) {
          ...
          await Audit.L10Log(s, caller, recurrenceId, "DeleteL10", ForModel.Create(r), r.Name);
          tx.Commit();
          s.Flush();
        }
      }
    ...
    public static async Task UndeleteL10Recurrence(UserOrganizationModel caller, long recurrenceId) {
      ...
      using (var s = HibernateSession.GetCurrentSession()) {
        using (var tx = s.BeginTransaction()) {
          ...
          r.DeleteTime = null;
          s.Update(r);
          await Audit.L10Log(s, caller, recurrenceId, "UndeleteL10", ForModel.Create(r), r.Name);
          tx.Commit();
          s.Flush();
        }
      }
          ...
          switch (startOrEnd.ToLower()) {
            ...
            case "end": {
                if (meeting.CompleteTime == null) {
                  throw new PermissionsException("Meeting has not been wrapped up.");
                }
    ...
    /// <returns></returns>
    public static async Task SetSharedVTOVision(ISession s, PermissionsUtility perms, long organizationId, long recurrenceId) {

      perms.ViewOrganization(organizationId);

      L10Recurrence found = null;
      if (recurrenceId != 0) {
        found = s.Get<L10Recurrence>(recurrenceId);
        if (organizationId != found.OrganizationId) {
          throw new PermissionsException("Incorrect Organization");
        }
        if (found == null) {
          throw new PermissionsException("Meeting not found");
        }
        perms.CanEdit(PermItem.ResourceType.L10Recurrence, recurrenceId, exceptionMessage: "Cannot share Business Plan from selected meeting (" + recurrenceId + ")");
      }

      var existing = s.QueryOver<L10Recurrence>().Where(x => x.DeleteTime == null && x.ShareVto == true && x.OrganizationId == organizationId).List().ToList();
      foreach (var e in existing) {
        perms.CanEdit(PermItem.ResourceType.L10Recurrence, e.Id, exceptionMessage: "Cannot unshare Business Plan from existing meeting (" + e.Id + ")");
        e.ShareVto = false;
        s.Update(e);
      }
      if (found != null) {

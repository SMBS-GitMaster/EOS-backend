...
namespace RadialReview.Accessors {
	public partial class L10Accessor : BaseAccessor {
		...
		#region Load Data
		public static void _LoadMeetingLogs_Unsafe(ISession s, params L10Meeting[] meetings) {
		...
		public static void _LoadMeetings_Unsafe(ISession s, bool loadUsers, bool loadMeasurables, bool loadRocks, params L10Meeting[] meetings) {
			var meetingIds = meetings.Where(x => x != null).Select(x => x.Id).Distinct().ToArray();
		...
		public static void _LoadRecurrences(ISession s, LoadMeeting load, params L10Recurrence[] all) {
			...
			if (recurrenceIds.Any()) {
                ...
                IEnumerable<MeetingConclusionModel> conclusionActionsQuery = null;
                ...
                if (load.LoadConclusionActions)
                {
                  conclusionActionsQuery = s.QueryOver<MeetingConclusionModel>()
                      .Where(x => x.DeleteTime == null)
                      .WhereRestrictionOn(x => x.L10Recurrence.Id).IsIn(recurrenceIds)
                      .Future<MeetingConclusionModel>();
                }
        ...
        List<L10Recurrence.L10Recurrence_Attendee> allAttend = null;
                ...
                List<MeetingConclusionModel> conclusionActions = null;
				if (load.LoadUsers)
                ...
                if (load.LoadConclusionActions)
                  conclusionActions = conclusionActionsQuery.ToList();



				//if (load.LoadAudio) {
				//	IEnumerable<long> rocksWithAudio = null;
				//	if (load.LoadRocks && allRocks.Any()) {
				//		rocksWithAudio = s.QueryOver<L10AudioAnchorModel>()
				//			.Select(Projections.Group<L10AudioAnchorModel>(x => x.ForModel.ModelId), Projections.RowCount())
				//			.WhereRestrictionOn(x => x.ForModel.ModelId).IsIn(allRocks.Select(x => x.Id).ToArray())
				//			.Where(x => x.DeleteTime == null && x.ForModel.ModelType == ForModel.GetModelType<RockModel>())
				//			.Future<object[]>()
				//			.Select(x => (long)x[0]);
				//	}
				...
				//	//Calculate
				//	if (rocksWithAudio != null) {
				//		foreach (var r in rocksWithAudio) {
				...
				foreach (var a in all.Where(x => x != null).ToList()) {
					...
					if (load.LoadPages) {
						a._Pages = allPages.Where(x => a.Id == x.L10RecurrenceId && x.DeleteTime == null).OrderBy(x => x._Ordering).ToList();
					}

                    if (load.LoadConclusionActions)
                      a.MeetingConclusion = conclusionActions.Where(x => a.Id == x.L10RecurrenceId && x.DeleteTime == null).FirstOrDefault();

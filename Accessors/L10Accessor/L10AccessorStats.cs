...
namespace RadialReview.Accessors {
	public partial class L10Accessor : BaseAccessor {
		...
		public static StatsDataGroup GetStatsData(UserOrganizationModel caller, long recurrenceId, DateRange range = null) {
			...
			range = range ?? new DateRange(today.AddDays(-7 * TimingUtility.STANDARD_SCORECARD_WEEKS), today);
			...
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					var todoData = new StatsData() {
						...
						Data = meetings.Where(x => x.TodoCompletion != null)
									...
									.Select(x => {
										...
										var den = x.TodoCompletion.Denominator;// x.Sum(y => y.TodoCompletion.Denominator);
										if (den == 0) {
											return new KeyValuePair<DateTime, decimal>();
										}

										var num = x.TodoCompletion.Numerator;// x.Sum(y => y.TodoCompletion.Numerator);

										return new KeyValuePair<DateTime, decimal>(x/*.First()*/.StartTime.Value.Date, num / den * 100);
									})
					};
					...
					var ratingData = new StatsData() {
						...
						Data = meetings.Where(x => x != null && x.AverageMeetingRating != null && x.AverageMeetingRating.IsValid())
						.Select(x => new KeyValuePair<DateTime, decimal>(x.StartTime.Value.Date, x.AverageMeetingRating.NotNull(y => y.GetValue(10))))
						//Data = ratings.GroupBy(x=>x.L10MeetingId)
						//			//.GroupBy(x => x.L10Meeting.StartTime.Value.StartOfWeek(weekStart))
						//			.SelectMany(x => {
						//				if (count == 0)
						//					return new List<KeyValuePair<DateTime, decimal>>();
			...
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					if (meeting == null || meeting.CompleteTime == null) {
						...
						todosCreated = s.QueryOver<TodoModel>().Where(x => x.ForRecurrenceId == recurrenceId && x.CreateTime > createTime && x.CreatedBy != null).List().ToList();
						allTodos = s.QueryOver<TodoModel>().Where(x => x.ForRecurrenceId == recurrenceId && x.CompleteTime == null && x.CreatedBy != null).List().ToList();
						oldTodos = s.QueryOver<TodoModel>().Where(x => x.ForRecurrenceId == recurrenceId && x.CreateTime < createTime && (x.CompleteTime == null || x.CompleteTime > createTime) && x.CreatedBy != null).List().ToList();
						version = 1;
						if (prevMeeting != null && prevMeeting.CompleteTime != null) {
							todoComplete = s.QueryOver<TodoModel>().Where(x => x.ForRecurrenceId == recurrenceId && x.CompleteTime > prevMeeting.CompleteTime && x.CreatedBy != null).List().Count;
						}
					} else {
						version = 2;
						...
						todosCreated = s.QueryOver<TodoModel>().Where(x => x.ForRecurrenceId == recurrenceId && x.CreateTime > meeting.CreateTime && x.CreateTime < meeting.CompleteTime && x.CreatedBy != null).List().ToList();
						allTodos = s.QueryOver<TodoModel>().Where(x => x.ForRecurrenceId == recurrenceId && x.CompleteTime == null && x.CreatedBy != null).List().ToList();
						oldTodos = s.QueryOver<TodoModel>().Where(x => x.ForRecurrenceId == recurrenceId && x.CreateTime < meeting.CreateTime && (x.CompleteTime == null || x.CompleteTime > meeting.CreateTime) && x.CreatedBy != null).List().ToList();
						if (prevMeeting != null && prevMeeting.CompleteTime != null) {
							todoComplete = s.QueryOver<TodoModel>().Where(x => x.ForRecurrenceId == recurrenceId && x.CompleteTime > prevMeeting.CompleteTime && x.CompleteTime < meeting.CompleteTime && x.CreatedBy != null).List().Count;
						}

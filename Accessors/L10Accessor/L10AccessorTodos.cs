...
namespace RadialReview.Accessors {
	public partial class L10Accessor : BaseAccessor {
		...
		private static List<TodoModel> GetTodosForRecurrence(ISession s, PermissionsUtility perms, long recurrenceId, long meetingId) {
			...
			var output = todoList.Fetch(x => x.AccountableUser).Eager
								 .Fetch(s => s.ForRecurrence.L10MeetingInProgress).Eager
								 .List()
								 .ToList();
			return output;
		}
    ...
    public static Dictionary<DateTime, int> GetCompletedTodoCountsForRecurrence(ISession s, PermissionsUtility perms, long recurrenceId, DateRange range)
    {
      perms.ViewL10Recurrence(recurrenceId);
      var dates = s.Query<TodoModel>().Where(
        _ => _.DeleteTime == null && _.ForRecurrenceId == recurrenceId && _.CompleteTime != null &&
        _.CompleteTime >= range.StartTime && _.CompleteTime <= range.EndTime).Select(_ => _.CompleteTime);


      Dictionary<DateTime, int> results = new Dictionary<DateTime, int>();
      foreach(var date in dates)
      {
        if(results.ContainsKey(date.Value))
        {
          results[date.Value]++;
        }
        else
        {
          results.Add(date.Value, 1);
        }
      }
    ...
    public static List<TodoModel> GetAllTodosForRecurrence(ISession s, PermissionsUtility perms, long recurrenceId, bool includeClosed = true, DateRange range = null, bool includeDeleted = false) {
			perms.ViewL10Recurrence(recurrenceId);
      var todoListQ = s.QueryOver<TodoModel>();

      if (!includeDeleted)
        todoListQ = s.QueryOver<TodoModel>().Where(x => x.DeleteTime == null && x.ForRecurrenceId == recurrenceId);
      else
        todoListQ = s.QueryOver<TodoModel>().Where(x => x.ForRecurrenceId == recurrenceId);
    ...
    public static List<TodoModel> GetAllTodosForRecurrence(UserOrganizationModel caller, long recurrenceId, bool includeClosed = true, DateRange range = null, bool includeDeleted = false) {
      using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					var perms = PermissionsUtility.Create(s, caller);

					return GetAllTodosForRecurrence(s, perms, recurrenceId, includeClosed, range, includeDeleted);
			}
		}
    }

        public static List<TodoModel> GetUserTodosForRecurrence(UserOrganizationModel caller, long userId, long recurrenceId, bool includeClosed = true, DateRange range = null)
        {
            using (var s = HibernateSession.GetCurrentSession())
            {
                using (var tx = s.BeginTransaction())
                {
                    ...
                    var todoListQ = s.QueryOver<TodoModel>().Where(x => x.DeleteTime == null && x.ForRecurrenceId == recurrenceId && x.AccountableUserId == userId);
                    if (range != null && includeClosed)
                    {
                        var st = range.StartTime.AddDays(-1);
                        var et = range.EndTime.AddDays(1);
        ...
        public static async Task UpdateTodoOrder(UserOrganizationModel caller, long recurrenceId, L10Controller.UpdateTodoVM model) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					await using (var rt = RealTimeUtility.Create(model.connectionId)) {
						...
						group.Call("setTodoOrder",model.todos);
					...
					await Audit.L10Log(s, caller, recurrenceId, "UpdateTodos", ForModel.Create<L10Recurrence>(recurrenceId));
					tx.Commit();
					s.Flush();
				}
			}

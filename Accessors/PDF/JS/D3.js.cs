	...
	public partial class D3 {
		...
		public partial class Layout {
			...
			public class node<N> : IChildren<N> where N : node<N> {
				...
				public virtual List<N> children { get; set; }
		...
		public partial class Layout {
			public class CompactTree<N> where N : node<N>, new() {
				...
				private void secondWalk(nodeWrapper<N> v) {
					var adj = 1;
					if (compactify && v._self._compact != null) {
						adj = 2;
					}
				...
				private double d3_layout_treeSeparation(nodeWrapper<N> a, nodeWrapper<N> b) {
					...
					var aCompactNull = a._self._compact == null;
					var bCompactNull = b._self._compact == null;

					if (!aCompactNull)
						p1 = a.NotNull(x => x._self._compact.originalParent);

					if (!bCompactNull)
						p2 = b.NotNull(x => x._self._compact.originalParent);
					...
					if (!bCompactNull) {
						isBLeft = b.NotNull(x => x._self._compact.side == "left");
						isBRight = b.NotNull(x => x._self._compact.side == "right");
					}

					if (!aCompactNull) {
						isALeft = a.NotNull(x => x._self._compact.side == "left");
						isARight = a.NotNull(x => x._self._compact.side == "right");
					}
					...
					if (!bCompactNull)
						bIsLeaf = b.NotNull(x => x._self._compact.isLeaf);
					if (!aCompactNull)
						aIsLeaf = a.NotNull(x => x._self._compact.isLeaf);


					if ((bIsLeaf && isBLeft && aCompactNull) ||
						(aIsLeaf && isARight && bCompactNull)) {
						a.SetDebugNotes("b");
						return w1_0;//1
					}

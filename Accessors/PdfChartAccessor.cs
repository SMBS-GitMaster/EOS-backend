...
namespace RadialReview.Accessors {
  public class PdfChartAccessor {
      ...
      if (feedbackRows.Any()) {
        ...
        foreach (var feedbackRowGroup in feedbackRows.GroupBy(x => x.GroupTitle)) {
          ...
          foreach (var included in feedbackRowGroup) {
            ...
            var iconRect = new XRect(placement.Left+feedbackGroupMargin, top, iconWidth, h + 1.5);
    ...
    public static XRect DrawRolesTable(XGraphics gfx, TermsCollection terms, XRect location, List<GetWantCapacityAnswer> answers, Unit? margin = null) {
      var state = gfx.Save();
      ...
      var gwcNames = new[] { terms.GetTerm(TermKey.Understand), terms.GetTerm(TermKey.Embrace), terms.GetTerm(TermKey.Capacity) };

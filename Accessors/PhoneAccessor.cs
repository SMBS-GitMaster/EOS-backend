...
namespace RadialReview.Accessors {
  public class PhoneAccessor : BaseAccessor {
    ...
    public static DefaultDictionary<string, string> PossibleActions(TermsCollection terms) {
      terms = terms ?? TermsCollection.DEFAULT;

      return new DefaultDictionary<string, string>(x => (x ?? "").ToTitleCase()){
        { PhoneAccessor.ISSUE, "Add "+terms.GetTerm(TermKey.Issues)  },
        { PhoneAccessor.TODO, "Add "+terms.GetTerm(TermKey.ToDos)},
        { PhoneAccessor.HEADLINE, "Add "+terms.GetTerm(TermKey.Headlines) },
      };
    }
    ...
    [Untested("Receive Text", "Weekly Meeting text", "PersonalText")]
      ...
      try {
          ...
          using (var tx = s.BeginTransaction()) {
            ...
            if (found == null) {
              ...
              if (found2 != null) {
                ...
                tx.Commit();
                s.Flush();
                return "Your phone has been registered. Add this number to your contacts to add " + found2.Action + "s via text message. Reply STOP to cancel.";
              }
        ...
        switch (found.Action) {
          case TODO:
            ...
            if (found.ForId == -2) {
              todoModel = TodoCreation.GeneratePersonalTodo(body, "-sent from phone", found.Caller.Id, now.AddDays(7), now, padId);
            } else {
              todoModel = TodoCreation.GenerateL10Todo(found.ForId, body, "-sent from phone", found.Caller.Id, now.AddDays(7), null, "TodoModel", -2, now, padId);
            }
            ...
            return "To-do added.";
          case ISSUE:
            var creation = IssueCreation.CreateL10Issue(body, "-sent from phone", found.Caller.Id, found.ForId, null, modelType: "IssueModel", modelId: -2, padId: padId);
            await IssuesAccessor.CreateIssue(found.Caller, creation);
            return "Issue added.";
          case HEADLINE: {

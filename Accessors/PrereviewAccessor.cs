...
namespace RadialReview.Accessors {
	public class PrereviewAccessor : BaseAccessor {
		public static void ManagerCustomizePrereview(UserOrganizationModel caller, long prereviewId, List<WhoReviewsWho> whoReviewsWho) {
		...
		[Obsolete("Fix for AC")]
		public static async Task CreatePrereview_Deprecated(UserOrganizationModel caller, long forTeamId, String reviewName, bool sendEmails,
			DateTime dueDate, DateTime preReviewDue, bool ensureDefault, bool anonFeedback) {
			...
			using (var s = HibernateSession.GetCurrentSession(singleSession: false)) {
				...
				using (var tx = s.BeginTransaction()) {
					var dataInteraction = ReviewAccessor.GetReviewDataInteraction_Deprecated(s, caller.Organization.Id);
		...
		public static PrereviewModel GetPrereview(UserOrganizationModel caller, long prereviewId) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
		...
		public static List<WhoReviewsWho> GetCustomMatches(UserOrganizationModel caller, long prereviewId) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					PermissionsUtility.Create(s, caller).ViewPrereview(prereviewId);
		...
		public static List<WhoReviewsWho> GetAllMatchesForReview(UserOrganizationModel caller, long reviewContainerId, List<WhoReviewsWho> defaultModel) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					var perms = PermissionsUtility.Create(s, caller);
		...
		public static void UnsafeExecuteAllPrereviews(ISession s, long reviewContainerId, DateTime now) {
			var prereviews = s.QueryOver<PrereviewModel>().Where(x => x.ReviewContainerId == reviewContainerId && x.DeleteTime == null).List().ToList();
			foreach (var prereview in prereviews) {
		...
		public static List<WhoReviewsWho> GetPreviousPrereviewForUser(UserOrganizationModel caller, long managerId, out string ReviewName) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {

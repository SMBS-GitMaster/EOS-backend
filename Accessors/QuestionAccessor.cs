...
namespace RadialReview.Accessors {
	public class QuestionAccessor : BaseAccessor {
		public static void SetQuestionsEnabled(UserOrganizationModel caller, long forUserId, List<long> enabled, List<long> disabled) {
		...
		public static ReviewModel GenerateReviewForUser(DataInteraction dataInteraction, PermissionsUtility perms, UserOrganizationModel reviewer, ReviewsModel reviewContainer, AskableCollection askables) {
		...
		public static List<QuestionModel> GetQuestionsForUser(UserOrganizationModel caller, long forUserId, DateRange range = null) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
		...
		public static QuestionCategoryModel GetCategory(UserOrganizationModel caller, long categoryId, Boolean overridePermissions) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
		...
		public static QuestionModel EditQuestion(UserOrganizationModel caller, long questionId, Origin origin = null, LocalizedStringModel question = null, long? categoryId = null, DateTime? deleteTime = null, QuestionType? questionType = null) {
			QuestionModel q;
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					caller = s.Get<UserOrganizationModel>(caller.Id);
		...
		public static QuestionModel GetQuestion(UserOrganizationModel caller, long id) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {

...
namespace RadialReview.Accessors {
	public class ReportAccessor {
		...
		public static async Task<ReportPdfModel> ArchiveReport(UserOrganizationModel caller, ReviewController.ReviewDetailsViewModel model,bool finalized, TermsCollection terms) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					PermissionsUtility.Create(s, caller).ViewReview(model.Review.Id);
					var settings = new PdfSettings(caller.Organization.Settings);
					var pdf = PdfAccessor.GenerateReviewPrintout(caller,terms, model, settings);

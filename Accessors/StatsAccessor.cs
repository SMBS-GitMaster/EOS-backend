                ...
                using (var tx = s.BeginTransaction()) {
                   ...
                   // LocalizedStringModel orgNameAlias = null;
                    ...
                    var dataF = s.QueryOver<UserOrganizationModel>()
                                    ...
                                    .Select(x => x.AttachTime, x => x.DeleteTime, x => x.CreateTime, x => userAlias.CreateTime, x => x.Id, x => x.Organization.Id, x => orgAlias.Name, x => orgAlias.AccountType, x => cache.LastLogin)
                                    .Future<object[]>()
                                    .Select(x => new {
            ...
            using (var s = HibernateSession.GetCurrentSession()) {
                using (var tx = s.BeginTransaction()) {
                    ...
                    var rockData = s.QueryOver<RockModel>()
                        ...
                        .ToList();
                    return GenerateBurndown("Outstanding Goal", rockData);
                }
            }
        }

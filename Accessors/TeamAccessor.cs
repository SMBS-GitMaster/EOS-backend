	...
	public class TeamAccessor : BaseAccessor {
		...
		public static List<OrganizationTeamModel> GetTeamsDirectlyManaged(UserOrganizationModel caller1, long userOrganizationId) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					var asUsers = new List<UserOrganizationModel>();
		...
		public static List<TeamDurationModel> GetUsersTeams(UserOrganizationModel caller, long forUserId) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					var perms = PermissionsUtility.Create(s, caller);
					return GetUsersTeams(s.ToQueryProvider(true), perms, forUserId, false);
				}
			}
		}
		public static List<long> GetUsersTeamIds(ISession s, PermissionsUtility perms, long forUserId) {
			var forUser = s.Get<UserOrganizationModel>(forUserId);
			perms.ViewOrganization(forUser.Organization.Id);

			OrganizationTeamModel orgTeamAlias = null;

			var teamsF = s.QueryOver<TeamDurationModel>()
							.JoinAlias(x => x.Team, () => orgTeamAlias)
							.Where(x => x.DeleteTime == null && x.UserId == forUserId)
							.Select(x => orgTeamAlias.Id)
							.Future<long>();

			var specialTeamsF = s.QueryOver<OrganizationTeamModel>()
								.Where(x => x.DeleteTime == null && x.Organization.Id == forUser.Organization.Id && (x.Type == TeamType.Managers || x.Type == TeamType.AllMembers))
								.Select(x => x.Type, x => x.Id)
								.Future<object[]>()
								.Select(x => new {
									Type = (TeamType)x[0],
									Id = (long)x[1]
			...
			var teams = teamsF.ToList();
			var specialTeams = specialTeamsF.ToList();

			var output = new List<long>();
			output.AddRange(teams);

			if (forUser.IsManager()) {
				var managerTeam = specialTeams.Where(x => x.Type == TeamType.Managers).FirstOrDefault();
				if (managerTeam != null)
					output.Add(managerTeam.Id);
			}

			var allMembersTeam = specialTeams.Where(x => x.Type == TeamType.AllMembers).FirstOrDefault();
			if (allMembersTeam != null) {
				output.Add(allMembersTeam.Id);
			}
		...
		public class UserIsManager {
			public UserIsManager(long userId, long orgId, bool isManager) {
				UserId = userId;
				OrgId = orgId;
				IsManager = isManager;
			}
		...
		public static IEnumerable<UserTeam> BulkGetUsersTeams_Unsafe(ISession s, List<UserIsManager> users) {
			var userIds = users.Select(x => x.UserId).ToArray();
			var allOrgIds = users.Select(x => x.OrgId).Distinct().ToArray();

			OrganizationTeamModel teamAlias = null;
			var teams = s.QueryOver<TeamDurationModel>()
							.JoinAlias(x => x.Team, () => teamAlias)
							.Where(x => x.DeleteTime == null && teamAlias.DeleteTime == null)
							.WhereRestrictionOn(x => x.UserId).IsIn(userIds)
							.Select(x => x.UserId, x => x.Team.Id)
							.Future<object[]>()
							.Select(x => new {
								UserId = (long)x[0],
								TeamId = (long)x[1],
			...
			var unfilteredOrgTeams = s.QueryOver<OrganizationTeamModel>()
							.Where(x => x.DeleteTime ==null && (x.Type == TeamType.Managers || x.Type == TeamType.AllMembers))
							.WhereRestrictionOn(x => x.Organization.Id).IsIn(allOrgIds)
							.Select(x => x.Id, x => x.Organization.Id, x => x.Type)
							.Future<object[]>()
							.Select(x => new {
								TeamId = (long)x[0],
								OrgId = (long)x[1],
			...
			return users.SelectMany(u => {
				var output = new List<UserTeam>();
				var allMembers = unfilteredOrgTeams.SingleOrDefault(y => y.Type == TeamType.AllMembers && y.OrgId == u.OrgId);
				if (allMembers != null) {
					output.Add(new UserTeam(u.UserId, allMembers.TeamId));
				}

				if (u.IsManager) {
					var mangersTeam = unfilteredOrgTeams.SingleOrDefault(y => y.Type == TeamType.Managers && y.OrgId == u.OrgId);
					if (mangersTeam != null) {
						output.Add(new UserTeam(u.UserId, mangersTeam.TeamId));
					}
				}
				var userTeams = teams.Where(x => x.UserId == u.UserId).Select(x => new UserTeam(u.UserId, x.TeamId));
				output.AddRange(userTeams);
				return output;
			});
		...
		public static List<TeamDurationModel> GetUsersTeams(AbstractQuery s, PermissionsUtility permissions, long forUserId, bool tryWithAlternateUsers) {
			var forUser = s.Get<UserOrganizationModel>(forUserId);
			if (tryWithAlternateUsers) {
				permissions.TryWithAlternateUsers(x => x.ViewOrganization(forUser.Organization.Id));
			} else {
		...
		public static bool AddMember(ISession s, PermissionsUtility perms, long teamId, long userOrgId) {
			perms.EditTeam(teamId).ViewUserOrganization(userOrgId, false);
			var team = s.Get<OrganizationTeamModel>(teamId);

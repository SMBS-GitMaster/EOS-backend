...
namespace RadialReview.Api.V1 {
	...
	public class TodosController : BaseApiController {
		...
		public TodosController(INotesProvider notesProvider, RadialReview.DatabaseModel.DatabaseContexts.RadialReviewDbContext dbContext, RedLockNet.IDistributedLockFactory redLockFactory) : base(dbContext, redLockFactory) {
		...
		public async Task EditTodo(long TODO_ID, [FromBody] UpdateTodoModel body) {
			...
			await TodoAccessor.UpdateTodo(GetUser(), TODO_ID, completeTime: DateTime.UtcNow, body.title, body.dueDate);
		}
		...
		/// <returns></returns>
		// PUT: api/Todo/5
		[Route("todo/{TODO_ID:long}/complete")]
		[HttpPost]
		//[HttpGet]
		public async Task<bool> MarkComplete(long TODO_ID, bool status = true) {
			...
			await TodoAccessor.CompleteTodo(GetUser(), TODO_ID, detachTime: DateTime.UtcNow, status);
			return true;
		...
		public AngularTodo Get(long TODO_ID) {
			return new AngularTodo(TodoAccessor.GetTodo(GetUser(), _notesProvider, TODO_ID));
		}
		...
		/// <returns></returns>
		[Route("todo/notes/{TODO_ID:long}")]
		[HttpGet]
		[ProducesResponseType(typeof(NotesPadResponse), StatusCodes.Status200OK)]
		public async Task<IActionResult> GetNotes(long TODO_ID, bool showControls = true, bool readOnly = false)
		{
			var todo = TodoAccessor.GetTodo(GetUser(), _notesProvider, TODO_ID);
			var padId = todo.PadId;
			if (readOnly || !PermissionsAccessor.IsPermitted(GetUser(), x => x.EditTodo(TODO_ID)))
			{
				padId = await _notesProvider.GetReadonlyUrl(todo.PadId);
			}

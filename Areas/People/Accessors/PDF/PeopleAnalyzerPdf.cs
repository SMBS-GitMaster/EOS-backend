...
namespace RadialReview.Areas.People.Accessors.PDF {
  public class PeopleAnalyzerPdf {
    ...
    /// <returns></returns>
    public static PdfDocumentRenderer AppendPeopleAnalyzer(ITimeData timeData, TermsCollection terms, Document doc, AngularPeopleAnalyzer pa, PdfSettings settings, DateTime? beforeDate = null) {
      ...
      if (pa == null || pa.Empty()) {
        var p = section.AddParagraph(terms.GetTerm(TermKey.RightPersonRightSeat)+" was empty");
        p.Format.Alignment = ParagraphAlignment.Center;
      ...
      if (container != null) {
        ...
        AddTitleText(timeData, terms, docRenderer, renderer, beforeDate.Value, font, new XPoint(leftMargin + FONT_SIZE, topMargin + FONT_SIZE));
      }

      return renderer;
    }

    private static void AddTitleText(ITimeData timeData, TermsCollection terms, DocumentRenderer docRenderer, PdfDocumentRenderer renderer, DateTime date, XFont font, XPoint position) {
      using (XGraphics gfx = XGraphics.FromPdfPage(renderer.PdfDocument.Pages[0])) {
        gfx.DrawString(terms.GetTerm(TermKey.RightPersonRightSeat)/*container.Name ?? ""*/, font, XBrushes.Black, position);
        ...
        if (date > DateTime.UtcNow)
          useDate = "as of " + timeData.ConvertFromServerTime(DateTime.UtcNow).Date.ToLongDateString();

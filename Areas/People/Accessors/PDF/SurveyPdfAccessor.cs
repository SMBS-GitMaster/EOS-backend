...
namespace RadialReview.Areas.People.Accessors.PDF {
  public class SurveyPdfAccessor {
    ...
    public static Color TractionBlue = new Color(38, 117, 169);
    public static Color TableWhite = Colors.White;// new Color(255, 255, 255);
    public static Color TractionBlack = new Color(52, 62, 69);// new Color(255, 255, 255);
    ...
    public static Document CreateDoc(UserOrganizationModel caller, string docTitle) {
      ...
      document.Info.Comment = "Created with Bloom Growth";
    ...
    public static void AppendSurveyAbout(Document doc, string title, DateTime localTime, ISurveyAbout survey, TermsCollection terms) {
      doc.DefaultPageSetup.StartingNumber = 1;
      var iSections = survey.GetSections().OrderBy(x => x.GetOrdering());
      var section = doc.AddSection();
      section.PageSetup.FooterDistance = BottomPad * 1;
      ...
      foreach (var iSection in iSections) {
        AddSurveySection(section, iSection, terms);
      }
    }
    ...
    protected static void AddSectionTitle(Section sec, string pageTitle, Unit usableWidth, Orientation orientation = Orientation.Portrait) {
      ...
      title.Borders.Color = TractionBlue;//TableBlack;
      ...
      rr.Shading.Color = TractionBlue;// TableGray;
      ...
      sec.Elements.Add(table);
    }

    protected static void AddSurveySection(Section section, ISectionAbout iSection, TermsCollection terms) {
      ...
      PdfSectionFactory.CreateSection(section, iSection, usableWidth, terms);
    }
  }

  public class PdfSectionFactory {

    public static bool CreateSection(Section section, ISectionAbout isection, Unit usableWidth, TermsCollection terms) {
      switch (isection.GetSectionType()) {
        case "Rocks":
          return AddRocksSection(section, isection, usableWidth, terms);
        case "Roles":
          return AddRolesSection(section,terms, isection, usableWidth);
        case "Values":
          return AddValuesSection(section, isection, usableWidth, terms);
        case "GeneralComments":
          return AddGeneralCommentsSection(section, isection, usableWidth);
        case "LeadershipAssessment":
          return AddLeadershipAssessmentSection(section, isection, usableWidth);
        case "ManagementAssessment":
          return AddManagementAssessmentSection(section, isection, usableWidth);
        case "RockCompletion":
          return AddRockCompletionSection(section, isection, usableWidth, terms);
        default:
    ...
    private static bool AddValuesSection(Section section, ISectionAbout iSection, Unit usableWidth, TermsCollection terms) {
      ...
      if (!valueQuestions.Any()) {
        var p = section.AddParagraph("No "+terms.GetTerm(TermKey.CoreValues)+" Responses.");
      ...
      DrawComments(section, ValueSection.ValueCommentHeading(terms), generalComments, usableWidth);
      return true;
      //if (generalComments.SelectMany(x => x.GetResponses().Where(y => !string.IsNullOrWhiteSpace(y.GetAnswer()))).Any()) {
    ...
    private static bool AddRockCompletionSection(Section section, ISectionAbout iSection, Unit usableWidth, TermsCollection terms) {
      ...
      p.AddFormattedText("# of "+terms.GetTerm(TermKey.Goals) +" Completed Last Quarter");
      ...
      p.Format.Alignment = ParagraphAlignment.Right;
      p.AddFormattedText("# of "+terms.GetTerm(TermKey.Goals) +" Last Quarter");
      p = row.Cells[2].AddParagraph();
      p.Format.Alignment = ParagraphAlignment.Center;
      ...
      p = row.Cells[1].AddParagraph();
      p.Format.Alignment = ParagraphAlignment.Right;
      p.AddFormattedText(terms.GetTermSingular(TermKey.Goals) +" Completion Percentage");
      p = row.Cells[2].AddParagraph();
      p.Format.Alignment = ParagraphAlignment.Center;
    ...
    private static bool AddRolesSection(Section section,TermsCollection terms, ISectionAbout iSection, Unit usableWidth) {
      ...
      if (rolesQuestions.Any()) {
        ...
        var gwc = new Dictionary<string, string> { { "get", terms.GetTerm(TermKey.Understand) }, { "want", terms.GetTerm(TermKey.Embrace) }, { "cap", terms.GetTerm(TermKey.Capacity) } };
    ...
    private static bool AddRocksSection(Section section, ISectionAbout iSection, Unit usableWidth, TermsCollection terms) {
      ...
      //Add the goal completion percentage
      ...
      if (!rockQuestions.Any()) {
        var p = section.AddParagraph("No "+terms.GetTerm(TermKey.Goals).ToLower()+" responses.");
        p.Format.Font.Color = _Gray;
        p.Format.Font.Italic = true;
        p.Format.Alignment = ParagraphAlignment.Center;
      }
      ...
      //ADD GENERAL COMMENTS
      var generalComments = questions.Where(x => x.GetFormat().GetQuestionIdentifier() == SurveyQuestionIdentifier.GeneralComment);
      DrawComments(section, RockSection.RockCommentHeading(terms), generalComments, usableWidth);

      return true;
    }

    private static bool AddGeneralCommentsSection(Section section, ISectionAbout iSection, Unit usableWidth) {

      var questions = iSection.GetItemContainers();
      var generalComments = questions.Where(x => x.GetFormat().GetQuestionIdentifier() == SurveyQuestionIdentifier.GeneralComment);

      return DrawComments(section, "General Comments", generalComments, usableWidth);

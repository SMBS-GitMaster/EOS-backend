...
#pragma warning disable CS0618 // Type or member is obsolete
		public static async Task JoinSurveyHub(IOuterSession s, PermissionsUtility perms, long callerId, long? surveyContainerId, long? surveyId, string connectionId) {
			await using (var rt = RealTimeUtility.Create()) {
				...
				if (surveyContainerId != null) {
					...
					await rt.AddToGroup(connectionId, RealTimeHub.Keys.GenerateSurveyContainerId(surveyContainerId.Value));
				}
				if (surveyId != null) {
					perms.ViewSurvey(surveyId.Value);
					await rt.AddToGroup(connectionId, RealTimeHub.Keys.GenerateSurveyId(surveyId.Value));
				}
				await rt.AddToGroup(connectionId, RealTimeHub.Keys.UserId(caller.Id));
			}
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		public static AngularSurveyContainer UpdateSurveyContainer(UserOrganizationModel caller, long surveyContainerId, string name = null, DateTime? dueDate = null) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					if (dueDate != null) {
						sc.DueDate = dueDate.Value.Date;
						foreach (var survey in surveysQ) {
							survey.DueDate = dueDate.Value.Date;
							s.Update(survey);
						}
						s.Update(sc);
					}
			...
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					foreach (var survey in surveys) {
						survey.ImageUrl = SurveyAccessor.TryGetImageUrlForSurvey(s, survey);
					}
		...
		private static IEnumerable<AngularSurveyContainer> GetSurveyContainerBy(UserOrganizationModel caller, IForModel byModel, SurveyType type, bool archivedOnly) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					var surveyData = s.QueryOver<Survey>()
						.Where(x => x.DeleteTime == null && x.SurveyType == type)
						.Where(x => x.By.ModelId == byModel.ModelId && x.By.ModelType == byModel.ModelType)
						//.Select(x => Projections.Group<Survey>(y => y.SurveyContainerId))
						.Select(x => x.SurveyContainerId, x => x.Id, x => x.Name)
						.List<object[]>().Select(x => new {
							containerId = (long)x[0],
							surveyId = (long)x[1],
							name = (string)x[2]
						}).ToArray();


					var containerIds = surveyData.Select(x => x.containerId).Distinct().ToArray();
					...
					var data = containers.Select(x => new AngularSurveyContainer(x,
																					x.DueDate < DateTime.UtcNow,
																					creatorLookup[x.CreatedBy.ToKey()])
					).ToList();

					foreach (var d in data) {
						d.Surveys = surveyData.Where(x => x.containerId == d.Id)
							.Select(x => new AngularSurvey(x.surveyId) {
								Name = x.name
							}).ToList();
					}
				...
				using (var tx = s.BeginTransaction()) {
					...
					var byAndLockedIn = s.QueryOver<Survey>()
						.Where(x => x.DeleteTime == null &&
									x.SurveyContainerId == surveyContainerId &&
									x.SendEmail)
			...
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					return surveys.Select(survey => {
						...
						var imgUrl = TryGetImageUrlForSurvey(s, survey);
						var ss = new AngularSurvey(survey, imgUrl) {
		...
		public static string TryGetImageUrlForSurvey(ISession s, ISurvey survey) {
			try {
				if (survey.GetAbout().Is<UserOrganizationModel>()) {
					return s.Get<UserOrganizationModel>(survey.GetAbout().ModelId).GetImageUrl();
				} else if (survey.GetAbout().Is<SurveyUserNode>()) {
					return s.Get<SurveyUserNode>(survey.GetAbout().ModelId).User.GetImageUrl();
				}
			} catch (Exception e) {
			}
			return null;

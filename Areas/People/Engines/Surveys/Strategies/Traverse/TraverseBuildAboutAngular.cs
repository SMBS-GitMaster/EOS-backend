...
namespace RadialReview.Areas.People.Engines.Surveys.Strategies.Traverse {
	public class TraverseBuildAboutAngular : TraverseBuild {
		...
		private Dictionary<string, string> GenLookups(ISurveyContainer surveyContainer) {
			...
			//	.Fetch(x => x._Users).Eager
			//	.Future();

			AccountabilityNode nodeAlias = null;
			var nodeMap = Session.QueryOver<AccountabilityNodeUserMap>()
									.JoinAlias(x => x.AccountabilityNode, () => nodeAlias)
									.Where(x => x.DeleteTime == null && nodeAlias.DeleteTime == null)
									.WhereRestrictionOn(x => x.AccountabilityNodeId).IsIn(nodeIds)
			...
			foreach (var u in nodeMap) {
				var name = u.User.NotNull(x => x.GetName());
				output[ForModel.Create(u.AccountabilityNode).ToKey()] = name;
			}

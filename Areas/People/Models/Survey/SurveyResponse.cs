...
namespace RadialReview.Areas.People.Models.Survey {
	...
	public class SurveyResponse : ILongIdentifiable, IHistorical, IResponse {
        ...
        [Audited(TargetAuditMode = RelationTargetAuditMode.NotAudited)]
        [NotAudited]
        public virtual SurveyItemFormat ItemFormat { get; set; }
        [Audited(TargetAuditMode = RelationTargetAuditMode.NotAudited)]
        [NotAudited]
        public virtual SurveyUserNode By_SUN { get; set; }
        [Audited(TargetAuditMode = RelationTargetAuditMode.NotAudited)]
        [NotAudited]
        public virtual SurveyUserNode About_SUN { get; set; }
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		public class Map : ClassMap<SurveyResponse> {
            public Map() {
                ...
                References(x => x.ItemFormat).Column("ItemFormatId").LazyLoad().ReadOnly();
                References(x => x.By_SUN).Column("By_ModelId").LazyLoad().ReadOnly();
                References(x => x.About_SUN).Column("About_ModelId").LazyLoad().ReadOnly();
            }
        }
    }
}
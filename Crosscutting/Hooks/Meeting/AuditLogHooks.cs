	...
	public class AuditLogHooks : ITodoHook, IRockHook, IHeadlineHook, IIssueHook, IMeetingRockHook, IMeetingMeasurableHook, IMeasurableHook {
		...
		public async Task CreateTodo(ISession s, UserOrganizationModel caller, TodoModel todo) {
			if (todo.ForRecurrenceId > 0) {
				await Audit.L10Log(s, todo.CreatedBy, todo.ForRecurrenceId.Value, "CreateTodo", ForModel.Create(todo), todo.NotNull(x => x.Message));
			}
		}
		...
		public async Task UpdateTodo(ISession s, UserOrganizationModel caller, TodoModel todo, ITodoHookUpdates updates) {
			if (todo.ForRecurrenceId > 0) {
				...
				await Audit.L10Log(s, caller, todo.ForRecurrenceId.Value, "UpdateTodo", ForModel.Create(todo), updatedText);
			}
		}
					...
					foreach (var recurrenceId in recurIds) {
						...
						if (currentMeeting != null) {
							...
							foreach (var r in meetingRocks) {
								await Audit.L10Log(s, caller, recurrenceId, "UpdateRockCompletion", ForModel.Create(r), "\"" + r.ForRock.Rock + "\" set to \"" + rock.Completion + "\"");
		...
		public async Task CreateHeadline(ISession s, UserOrganizationModel caller, PeopleHeadline headline) {
			...
			if (recurrenceId > 0) {
				await Audit.L10Log(s, s.Get<UserOrganizationModel>(headline.CreatedBy), headline.RecurrenceId, "CreateHeadline", ForModel.Create(headline), headline.NotNull(x => x.Message));
			}
		}
		...
		public async Task CreateIssue(ISession s, UserOrganizationModel caller, IssueModel.IssueModel_Recurrence issue) {
			...
			if (recurrenceId > 0) {
				await Audit.L10Log(s, issue.CreatedBy, recurrenceId, "CreateIssue", ForModel.Create(issue.Issue), issue.Issue.NotNull(x => x.Message));
			}
		}
      ...
      if (updates.StarsChanged)
        updatesText.Add("Stars from " + updates.oldStars + " to " + issueRecurrence.Stars);
      ...
      if (updates.CompletionChanged) {
			...
			await Audit.L10Log(s, caller, recurrenceId, "UpdateIssue", ForModel.Create(issueRecurrence), updatedText);
		}
		...
		public async Task AttachRock(ISession s, UserOrganizationModel caller, RockModel rock, L10Recurrence.L10Recurrence_Rocks recurRock) {
			await Audit.L10Log(s, caller, recurRock.L10Recurrence.Id, "CreateRock", ForModel.Create(recurRock), rock.Rock);
		}
		...
		public async Task AttachMeasurable(ISession s, UserOrganizationModel caller, MeasurableModel measurable, L10Recurrence.L10Recurrence_Measurable recurMeasurable) {
			await Audit.L10Log(s, caller, recurMeasurable.L10Recurrence.Id, "CreateMeasurable", ForModel.Create(measurable), measurable.Title);
		}

		public async Task DetachMeasurable(ISession s, UserOrganizationModel caller, MeasurableModel measurable, long recurrenceId) {
			await Audit.L10Log(s, caller, recurrenceId, "DeleteMeasurable", ForModel.Create(measurable), measurable.Title);
		}
		...
		public async Task UpdateMeasurable(ISession s, UserOrganizationModel caller, MeasurableModel measurable, List<ScoreModel> updatedScores, IMeasurableHookUpdates updates) {
			...
			if (updates.ShowAverageChanged)
				updateText.Add("Average: " + measurable.ShowAverage);

			if (updates.AverageRangeChanged)
				updateText.Add("Average Start: " + measurable.AverageRange);
			...
			foreach (var recurrenceId in recurrenceIds) {
				await Audit.L10Log(s, caller, recurrenceId, "UpdateArchiveMeasurable", ForModel.Create(measurable), updatedText);
			}
		}

			...
			foreach (var asanaTaskId in asanaTaskIds) {
				try {
					...
					if (token != null && token.AccessToken != null) {
						...
						await TodoAccessor.UpdateTodo(caller, todoId, completeTime: completeTime, asanaTask.name, asanaTask.due_at, null, null, asanaTask.completed, updateSource: UPDATE_SOURCE);
					}
				} catch (Exception e) {

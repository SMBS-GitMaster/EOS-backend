...
namespace RadialReview.Utilities.DataTypes {
	...
	public class Rect : IEnumerable<Tuple<int, int>> {
		...
		public RectType EnsureRowOrColumnOrCell() {
			var type = GetRectType();
			if (type == RectType.Rectangle && !IsCell())
				throw new ArgumentOutOfRangeException("rect", "Must be a row or column or cell");
			return type;
		}
		...
		public void EnsureSameRangeAs(Rect other) {
			...
			if (type == other.GetRectType()) {
				...
				} else if (type == RectType.Cell) {
					if (rect[0] != other.rect[0] && rect[1] != other.rect[1])
						throw new ArgumentOutOfRangeException("rect", "Cells must have the same range");

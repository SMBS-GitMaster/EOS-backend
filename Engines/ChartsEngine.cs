...
namespace RadialReview.Engines {
	public class ChartsEngine {
		...
		public static String GetChartTitle(UserOrganizationModel caller, long chartTupleId) {
		...
		public static ScatterPlot ScatterFromOptions(UserOrganizationModel caller, ChartOptions options, bool sensitive) {
		...
		protected static ScatterPlot ReviewScatterFromOptions(UserOrganizationModel caller, ChartOptions options, bool sensitive) {
		...
		public static Scatter AggregateReviewScatter(UserOrganizationModel caller, long reviewsId, bool admin) {
			long orgId;
		...
		public static List<Scatter.ScatterPoint> GenerateLegend() {
		...
		public static Scatter ReviewScatter2(UserOrganizationModel caller, long forUserId, long reviewsId, string groupBy, bool sensitive, bool includePrevious) {
			...
			var reviewAnswers = ReviewAccessor.GetAnswersForUserReview_Scatter(caller, forUserId, reviewsId);
			...
			if (previousReview != null) {
				string temp = null;
				var previousReviewAnswers = ReviewAccessor.GetAnswersForUserReview_Scatter(caller, forUserId, previousReview.Value);
		...
		public static ScatterPlot ReviewScatter(UserOrganizationModel caller, long forUserId, long reviewsId, bool sensitive) {
			if (sensitive) {
				PermissionsAccessor.Permitted(caller, x => x.ManagesUserOrganization(forUserId, true));
			}

			var categories = OrganizationAccessor.GetOrganizationCategories(caller, caller.Organization.Id);
			var review = ReviewAccessor.GetAnswersForUserReview(caller, forUserId, reviewsId);
			var history = new List<AnswerModel>();
			if (true)//includeHistory)
			{
				history = ReviewAccessor.GetAnswersAboutUser(caller, forUserId);
			}
			...
			foreach (var userAnswers in groupedByUsers) //<UserId>
			{
				...
				foreach (var userReviewAnswers in byReviewOrdered) {
					...
					var reviewContainer = ReviewAccessor.GetReviewContainer(caller, reviewContainerId, false, false, false);
		...
		public static Line ScorecardChart(UserOrganizationModel caller, long measureableId) {

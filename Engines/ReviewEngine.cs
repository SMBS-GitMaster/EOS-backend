...
namespace RadialReview.Engines {
	public class ReviewEngine : BaseEngine {
		...
		public static List<ReviewsModel> Filter(List<UserOrganizationModel> subordinates, List<ReviewsModel> allReviews) {
			return allReviews.Where(x => x.Reviews.Any(y => subordinates.Any(z => z.Id == y.ReviewerUserId))).ToList();
		}


		[Obsolete("fix me", true)]
		public static CustomizeModel GetCustomizeModel(UserOrganizationModel caller, long teamId, bool includeCopyFrom, DateRange dateRange = null, List<WhoReviewsWho> existingMatches = null) {
			...
			if (includeCopyFrom) {
				...
				var copy = PrereviewAccessor.GetPreviousPrereviewForUser(caller, caller.Id, out reviewName);
				if (reviewName != null) {
					//last review
		...
		[Obsolete("Fix for AC")]
		public static async Task<int> CreateReviewFromPrereview(NexusModel nexus) {
			try {
				var admin = UserOrganizationModel.ADMIN;
				var reviewContainerId = nexus.GetArgs()[0].ToLong();

				return await CreateReviewFromPrereview_Unsafe(admin, reviewContainerId);
			} catch (Exception) {
		...
		[Obsolete("Fix for AC")]
		public static async Task<int> CreateReviewFromPrereview(UserOrganizationModel caller, long reviewContainerId) {
			PermissionsAccessor.EnsurePermitted(caller, x => x.AdminReviewContainer(reviewContainerId));
			return await CreateReviewFromPrereview_Unsafe(caller, reviewContainerId);
		}


		[Obsolete("broken", true)]
		private static async Task<int> CreateReviewFromPrereview_Unsafe(UserOrganizationModel admin, long reviewContainerId) {
			//var prereview = _PrereviewAccessor.GetPrereview(admin, prereviewId);
			var now = DateTime.UtcNow;
			var reviewContainer = ReviewAccessor.GetReviewContainer(admin, reviewContainerId, false, false);
			...
			var whoReviewsWho = PrereviewAccessor.GetAllMatchesForReview(admin, reviewContainerId, defaultCustomize);
			var organization = OrganizationAccessor.GetOrganization(admin, reviewContainer.OrganizationId);
			var unsentEmail = new List<Mail>();

			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					//20210130 to-finish
					var datainteraction = ReviewAccessor.GetReviewDataInteraction_Deprecated(s, reviewContainer.OrganizationId);
					var perm = PermissionsUtility.Create(s, admin);
					unsentEmail = ReviewAccessor.CreateReviewFromPrereview(s, datainteraction, perm, admin, reviewContainer, whoReviewsWho);
					PrereviewAccessor.UnsafeExecuteAllPrereviews(s, reviewContainerId, now);
					//Keep these:
					tx.Commit();
					s.Flush();
				}

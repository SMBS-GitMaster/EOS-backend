...
namespace RadialReview.Models.Attributes {
	...
	public class IndexInitializer<T> : IDatabaseInitializer<T> where T : DbContext {
		...
		public void InitializeDatabase(T context) {
			...
			foreach (PropertyInfo dataSetProperty in typeof(T).GetProperties(PublicInstance).Where(p => p.PropertyType.Name == typeof(DbSet<>).Name)) {
				...
				foreach (IndexAttribute indexAttribute in indexes.Keys) {
					query += CreateIndexQueryTemplate.Replace("{indexName}", indexAttribute.Name).Replace("{tableName}", tableName).Replace("{columnName}", string.Join(", ", indexes[indexAttribute].ToArray())).Replace("{unique}", indexAttribute.IsUnique ? "UNIQUE" : string.Empty);
				}
			}
		...
		IEnumerable<IndexAttribute> GetIndexAttributes(PropertyInfo property) {
			...
			var indexAttributes = (IndexAttribute[])property.GetCustomAttributes(typeof(IndexAttribute), false);
			var metadataAttribute = entityType.GetCustomAttribute(typeof(MetadataTypeAttribute)) as MetadataTypeAttribute;
			...
			var associatedIndexAttributes = (IndexAttribute[])associatedProperty.GetCustomAttributes(typeof(IndexAttribute), false);

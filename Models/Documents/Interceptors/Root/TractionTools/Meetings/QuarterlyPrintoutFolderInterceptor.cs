...
namespace RadialReview.Models.Documents.Interceptors {
  public class QuarterlyPrintoutFolderInterceptor : IDocumentFolderInterceptor_Unsafe {
    public async Task<List<DocumentItemVM>> GeneratedDocumentItems(ISession s, UserOrganizationModel caller, DocumentsFolder folder) {
      ...
      var tagLookup = s.QueryOver<EncryptedFileTagModel>()
        .Where(x => x.DeleteTime == null && x.Tag == TagModel.Constants.QUARTERLY_PRINTOUT)
      ...
      var qpFiles = files.Where(x => tagLookup[x.Id].Any()).ToList();


      var dedup = FileNameUtility.CreateNameDeduplicator();
      return qpFiles.OrderBy(x => x.CreateTime).Select(x => {
    ...
    public bool ShouldExecute(ISession s, DocumentsFolder folder) {
      return FolderConsts.QuarterlyPrintoutFolder.InterceptorMatches(folder);
    }

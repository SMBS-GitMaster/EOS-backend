...
namespace RadialReview {
	...
	public static class NumberFormatExtensions {
		public static List<SelectListItem> Formats() {
			return Enum.GetValues(typeof(NumberFormat)).Cast<NumberFormat>().Select(x => new SelectListItem() { Text = x.Sample(), Value = "" + x }).ToList();
		}
		...
		public static string Radix(this NumberFormat format) {
			switch (format) {
				case NumberFormat.English:
					return ".";
				case NumberFormat.European1:
					return ",";
				case NumberFormat.European2:
					return ",";
				default:
		...
		private static Dictionary<NumberFormat, string> GetAngFormats() {
			return Enum.GetValues(typeof(NumberFormat)).Cast<NumberFormat>().ToDictionary(x => x, x => {
				var r = x.Radix();

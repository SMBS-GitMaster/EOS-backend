...
namespace RadialReview.Models.Reviews {
	...
	public class Reviewee {
		...
		[Obsolete("needs to split by user", true)]
	...
	public class Reviewer {
		...
		public Reviewer(AccountabilityNode reviewer, long reviewerUserId) : this(new AngularAccountabilityNode(reviewer), reviewerUserId) { }
		[Obsolete("fix me", true)]
		public Reviewer(AngularAccountabilityNode reviewer, long reviewerUserId) : this(reviewerUserId) {
			_Name = reviewer.Users.Where(x => x.Id == reviewerUserId).FirstOrDefault().NotNull(x => x.Name);

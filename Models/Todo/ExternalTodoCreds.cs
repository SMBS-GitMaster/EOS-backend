...
namespace RadialReview.Models.Todo {
	...
	public abstract class AbstractTodoCreds : ILongIdentifiable, IHistorical {
		...
		public abstract Task<bool> AddTodo(ISession s, INotesProvider notesProvider, TodoModel details);
	...
	public class TrelloTodoCreds : AbstractTodoCreds {
		...
		public override async Task<bool> AddTodo(ISession s, INotesProvider notesProvider, TodoModel todo) {
			return false;
		}
		...
		public override async Task<bool> AddTodo(ISession s, INotesProvider notesProvider, TodoModel todo) {
			...
			if (!String.IsNullOrWhiteSpace(todo.Details) && !String.IsNullOrWhiteSpace(newId)) {
				...
				using (var streamWriter = new StreamWriter(httpWebRequest2.GetRequestStream())) {
					...
					var padDetails = await notesProvider.GetTextForPad(todo.PadId);

					var json = @"{""content"": """ + padDetails.EscapeJSONString() + @""",""subject"": ""Details""}";
					streamWriter.Write(json);
					streamWriter.Flush();
					streamWriter.Close();
				}

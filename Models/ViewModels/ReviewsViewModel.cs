...
namespace RadialReview.Models.ViewModels {
	public class ReviewsViewModel {
		...
		public List<HtmlString> DropdownLinks { get; set; }
		...
		public ReviewsViewModel(ReviewsModel review) {
			...
			DropdownLinks = new List<HtmlString>();
		}
		...
		public void AddLink(string href, string text, string iconClass = "", string linkClass = "") {
			DropdownLinks.Add(new HtmlString("<a class='" + linkClass + "' href='" + href + "'><span class='" + iconClass + "'></span> " + WebUtility.HtmlEncode(text) + "</a>"));
		}

		public void AddAction(string javascript, string text, string iconClass = "", string linkClass = "") {
			DropdownLinks.Add(new HtmlString("<a class='" + linkClass + "' href='#' onclick='" + javascript + "'><span class='" + iconClass + "'></span> " + WebUtility.HtmlEncode(text) + "</a>"));
		}
		...
		public HtmlString GetTitle() {
			if (IsPrereview) {
				return new HtmlString("<a href='/Prereview/Customize/" + Prereview.Id + "' >" + Review.ReviewName + "<small></small></a>");
			} else {
				if (TakableId != null) {
					return new HtmlString("<a href='/Review/Take/" + Review.Id + "' >" + Review.ReviewName + "</a>");
				} else {
					return new HtmlString(Review.ReviewName);
				}
			}
		}

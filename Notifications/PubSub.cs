...
namespace RadialReview.Notifications {
	...
	public class PubSub {
		...
		[Obsolete("broken", true)]
		public static async Task SetSeenStatus(UserOrganizationModel caller, long userId, long notificationId, bool seen, string connectionId = null) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
		...
		public static void MarkSeen(ISession s, PermissionsUtility perms, RealTimeUtility rt, long userId, long notificationId) {
			...
			if (!any) {
				...
				var self = s.Get<UserOrganizationModel>(userId);
				if (self != null && self.User != null) {
					rt.UpdateOrganization(notification.OrganizationId).Call("NotificationStatus", notification.Id, true, self.User.UserName);
				}
		...
		public static List<Notification> ListUnseen(ISession s, PermissionsUtility perms, long userId) {
			...
			var groups = ResponsibilitiesAccessor.GetResponsibilityGroupIdsForUser(s, perms, userId).ToArray();
			...
			var notifications = s.QueryOver<Notification>()
				.Where(x => x.DeleteTime == null && x.AllSeen == false) 
				.Where(ors)

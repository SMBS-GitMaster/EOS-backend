...
namespace RadialReview.Utilities {
	...
	public class DistanceUtility {
		...
		public static NameFormat _DetermineNameFormat(IEnumerable<string> names, IEnumerable<TinyUser> available_first_last_id, Dictionary<string, List<string>> nicknameCache, int thresh = 2) {
			...
			if (format == NameFormat.FN && histogramCounts.Any(x => x.Key == NameFormat.FNLI))
				format = NameFormat.FNLI;
			if (format == NameFormat.FN && histogramCounts.Any(x => x.Key == NameFormat.FNLN))
				format = NameFormat.FNLN_FN;
			if (format == NameFormat.LN && histogramCounts.Any(x => x.Key == NameFormat.FILN))
				format = NameFormat.FILN;
			if (format == NameFormat.LN && histogramCounts.Any(x => x.Key == NameFormat.FNLN))
				format = NameFormat.FNLN_LN;

...
namespace RadialReview.Utilities.Constants
{
	public class KeyManager
	{
		...
		public static DatabaseCredentials ProductionDatabaseCredentials
		{
			get
			{
				var secretName = Config.GetDatabaseCredentialsSecretName();
				var s = GetSecret(secretName);
		...
		private static Key GetSecret_CacheMiss(string secretName)
		{
			...
			try
			{
				Debug.WriteLine("user" + client.Config.AuthenticationServiceName.ToString());
				response = client.GetSecretValueAsync(request).GetAwaiter().GetResult();
				Debug.WriteLine("response");
			}
			catch (ResourceNotFoundException)
			{
				Debug.WriteLine("The requested secret {0} was not found", secretName);
				throw;
			}
			catch (InvalidRequestException e)
			{
				Debug.WriteLine("The request was invalid due to: {0}", e.Message);
				throw;
			}
			catch (InvalidParameterException e)
			{
				Debug.WriteLine("Request had invalid params: {0}", e.Message);
				throw;
			}
			catch (InternalServiceErrorException e)
			{
				Debug.WriteLine("An error occurred on the server side.", e.Message);
				throw;
			}
			catch (DecryptionFailureException e)
			{
				Debug.WriteLine("Secrets Manager can't decrypt the protected secret text using the provided KMS key.", e.Message);
				throw;
			}
			...
			Console.WriteLine("secret " + secretName.ToString());

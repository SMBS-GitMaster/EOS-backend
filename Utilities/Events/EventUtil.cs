	...
	public class EventUtil {
		...
		public class TriggerOptions {
			...
			public TriggerOptions Create(ISession s, EventType type, UserOrganizationModel caller, ILongIdentifiable model, String message = null, decimal? arg1 = null) {
				return Create(s, type, caller, caller.NotNull(x => x.Organization.NotNull(y => y.Id)), ForModel.Create(model), message, arg1);
			}
		...
		[Queue(HangfireQueues.Immediate.DAILY_ORGANIZATION_EVENTS)]
		[AutomaticRetry(Attempts = 0)]
		public static async Task<object> GenerateOrganizationDailyEvents_Hangfire(long orgId, AccountType type, DateTime? deleteTime, DateTime now) {
			if (deleteTime != null || type == AccountType.Cancelled || type == AccountType.Dormant && type == AccountType.Other) {
				return new {
					Executed = false,
					OrgId = orgId,
					AccountType = type,
			...
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					name = o.GetName();
					var orgLookupsQ = s.QueryOver<OrganizationLookup>().Where(x => x.OrgId == orgId).Select(x => x.OrgId, x => x.LastUserLoginTime, x => x.CreateTime).Future<object[]>();
					var recursQ = s.QueryOver<L10Recurrence>().Where(x => x.DeleteTime == null && x.OrganizationId == orgId).Select(x => x.Id, x => x.TeamType, x => x.CreateTime, x => x.OrganizationId).Future<object[]>();
					var meetingsQ = s.QueryOver<L10Meeting>().Where(x => x.DeleteTime == null && x.OrganizationId == orgId).Select(x => x.Id, x => x.L10RecurrenceId, x => x.StartTime, x => x.CompleteTime, x => x.OrganizationId).Future<object[]>();
					var reviewsQ = s.QueryOver<ReviewsModel>().Where(x => x.DeleteTime == null && x.OrganizationId == orgId).Select(x => x.Id, x => x.DateCreated, x => x.OrganizationId).Future<object[]>();
					var eventsQ = s.QueryOver<AccountEvent>().Where(x => x.DeleteTime == null && x.OrgId == orgId).Select(x => x.Id, x => x.CreateTime, x => x.Type, x => x.OrgId).Future<object[]>();
					...
					potentialEvents = potentialEvents.Where(x => x.Item2 != null).ToList();
			...
			var output = potentialEvents.GroupBy(x => x.Item2).ToDictionary(x => x.Key, x => x.Count());
			return new {
				Executed = true,
				OrgId = orgId,
				AccountType = type,
		...
		public static async Task GenerateAllDailyEvents_Hangfire(DateTime now) {
			...
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					var orgsQ = s.QueryOver<OrganizationModel>().Select(x => x.Id, x => x.AccountType, x => x.DeleteTime)
						.List<object[]>()
						.Select(x => new {
							Id = (long)x[0],
							AccountType = (AccountType)x[1],
							DeleteTime = ((DateTime?)x[2])
						}).ToList();

					foreach (var o in orgsQ) {
						Scheduler.Enqueue(() => GenerateOrganizationDailyEvents_Hangfire(o.Id, o.AccountType, o.DeleteTime, now));
					}
				}
			}
			return;

	...
	public static class AboutTypeExtensions {
		...
		public static AboutType Invert(this AboutType self) {
			...
			foreach (AboutType flag in self.GetFlags()) {
				switch (flag) {
					case AboutType.Invalid:
						build = build | AboutType.Invalid;
						break;
					case AboutType.Manager:
						build = build | AboutType.Subordinate;
						break;
					case AboutType.NoRelationship:
						build = build | AboutType.NoRelationship;
						break;
					case AboutType.Peer:
						build = build | AboutType.Peer;
						break;
					case AboutType.Self:
						build = build | AboutType.Self;
						break;
					case AboutType.Subordinate:
						build = build | AboutType.Manager;
						break;
					case AboutType.Teammate:
						build = build | AboutType.Teammate;
						break;
					case AboutType.Organization:
						build = build | AboutType.Organization;
						break; // The Organization Selector in GetCustomizeModel() depends on this inversion
					default:
						throw new ArgumentException("Unknown about type (" + self + ")");
				}
			}

	...
	public class FormulaUtility {
		...
		public class ParsedFormula {
			...
			public double? Evaluate(Func<string, double?> replacements, bool nullOnDivideByZero = true) {
				...
				var sb = Replace(x => "(" + (replacements(x) ?? 0.00) + ")");

				var result = Calculator.Calculate(sb);

				//TT-103: Do not include the current week 0 in the average.
				//All formula values are null and the results is zero, then use NULL instead.
				bool isNullResult = Parts.Where(p => p.StartsWith("[")).ToList().TrueForAll(p => {
					return replacements(p.Substring(1, p.Length - 2)) == null;
				});
				if (isNullResult && result.Result == 0.0) {
					return null;
				}

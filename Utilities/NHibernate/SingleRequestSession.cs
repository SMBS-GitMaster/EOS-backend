...
namespace RadialReview.Utilities.NHibernate {
  ...
  public interface IOuterSession : ISession {
    bool IsIsolated { get; }
    Guid SessionId { get { return this.GetSessionImplementation().SessionId; } }
  }

  public class SingleRequestSession : IOuterSession, ISession {
    ...
    public class Context {
      ...
      public Context(SingleRequestSession self) {
        ...
        StackTrace = Debug.GenerateStackTrace();
      }
    ...
    public void RunAfterDispose(OnDisposedModel onDisposed) {
      if(IsInsideSession()) {
        ...
        OuterSessionDisposeActions.Add(onDisposed);
      }
    }


    private List<OnDisposedModel> OuterSessionDisposeActions = new List<OnDisposedModel>();
    private Stack<Context> _ContextStack { get; set; }
    private ISession _backingSession;
    private bool _shouldDispose;
    private bool _backingDisposed;
    private string _ctorStackTrace;
    private string _disposeStackTrace;

    public bool WasDisposed { get; set; }
    public bool IsIsolated { get; private set; }
    public bool SerialAccess { get; private set; }
    ...
    public SingleRequestSession(ISession toWrap, bool dispose = false) {
      GUID = "" + Guid.NewGuid();
      Debug.BeginStack("Opening +" + GUID.Substring(0, 4));

      _ctorStackTrace = Debug.GenerateStackTrace();
    ...
    ~SingleRequestSession() {
      if(!_backingDisposed) {
        Debug.WriteLine("Failed to dispose " + GUID);
        Debug.WriteLine(_ctorStackTrace);
      }
    }
    ...
    public void AddContext() {
      if(_backingDisposed) {
        Debug.WriteLine("trying to add context to disposed SingleRequestSession");
        Debug.WriteLine(_disposeStackTrace);
        int a = 0;
      }
    ...
    public bool IsInsideSession() {
      return !_ContextStack.IsEmpty();
    }

    public void OuterDispose() {
      //We're outside a session, probably want to run these on the outer session.
      var outerSessionActions = OuterSessionDisposeActions.ToList();
      OuterSessionDisposeActions.Clear();
      foreach(var a in outerSessionActions) {
        HookExecutionService.RunAsync(a.Action);
      }
    }
    ...
    public void Dispose() {
      ...
      //Process dispose of backing session
      if(_shouldDispose) {
        DisposeBackingSession();
      } else {
        Debug.WriteLine("Skip Close +" + GUID.Substring(0, 4));
      }
    ...
    public void DisposeBackingSession() {
      Debug.EndStack("Closing +" + GUID.Substring(0, 4));
      _disposeStackTrace = Debug.GenerateStackTrace();

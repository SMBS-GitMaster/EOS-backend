...
namespace RadialReview.Utilities.Pdf {
	...
	public class DocumentMerger {
		...
		public void AddDoc(StreamAndMeta doc) {
			docs.Add(doc);
		}
		...
		public PdfDocument Flatten(string title, bool includeNumber, bool includeDate = true, string dateFormat = null, string name = null) {
			...
			document.Info.Keywords = "Bloom Growth";
		...
		protected void RenderPage(PdfDocument parentDoc, object doc, string name,ref int? pageNumber, bool includeNumber, bool includeDate, string dateFormat, XFont font, DateTime now) {
			...
			} else if (doc is PdfDocumentRenderer) {
				RenderPdfRenderer(parentDoc, name, ref pageNumber, includeNumber, includeDate, dateFormat, font, now, (PdfDocumentRenderer)doc);
			} else if (doc is StreamAndMeta) {
				RenderStreamAndMetaRenderer(parentDoc, name, ref pageNumber, includeNumber, includeDate, dateFormat, font, now, (StreamAndMeta)doc);
			} else {
				throw new NotImplementedException("Unknown pdf type");
			}
		}
		private void RenderStreamAndMetaRenderer(PdfDocument parentDoc, string name, ref int? pageNumber, bool includeNumber, bool includeDate, string dateFormat, XFont font, DateTime now, StreamAndMeta mdoc) {
			PdfDocument newPdfDoc;
			using (var stream = new MemoryStream()) {
				newPdfDoc = PdfReader.Open(mdoc.Content, PdfDocumentOpenMode.Import);
			}

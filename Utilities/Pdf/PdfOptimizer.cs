...
namespace RadialReview.Utilities
{
	...
	public class RangedVariables
	{
		...
		public RangedVariables CloneWith(Vector<double> values)
		{
			return CloneWith(values.ToArray());
		}
	...
	public class ViewBoxOptimzer
	{
		...
		public static ViewBoxOptimzerResults Optimize(Unit setToHeight, Unit width, IEnumerable<IElement> elements, RangedVariables v, TimeoutCheck timeout)
		{
			try
			{
				...
				var solver = new NelderMeadSimplex(.01, 2000);


				var itr = 0;
				var solution = solver.FindMinimum(ObjectiveFunction.Value(x => {

					if (itr++ % 100 == 0) { timeout.ShouldTimeout(); }

					var clone = v.CloneWith(x);
					var fit = FitHeights(setToHeight, width, elements, clone);
					var err = fit.SqrError;
					return err;
				}), CreateVector.DenseOfArray(initial.ToArray())/*, lowers, uppers*/);

				var valid = new[] {
				ExitCondition.RelativePoints,
				ExitCondition.RelativeGradient,
				ExitCondition.AbsoluteGradient,
				ExitCondition.WeakWolfeCriteria,
				ExitCondition.BoundTolerance,
				ExitCondition.StrongWolfeCriteria,
				...
				{
					optimal.Add(solution.MinimizingPoint[i]);
				}

				var solverCompleted = valid.Any(x => x == solution.ReasonForExit);
				...
				for (var i = 0; i < 2; i++)
				{
					...
					res.Error = solution.FunctionInfoAtMinimum.Value;
			...
			{
				throw new LayoutTimeoutException("Maximum interations exceeded", e);
			}
		}
		...
		public static LayoutOptimizerResults Optimize(IDocumentGenerator layoutGenerator, IEnumerable<IHint> hints, RangedVariables vars, TimeoutCheck timeout)
		{
			...
			while (true)
			{
				...
				foreach (var viewBox in pageViewBoxes)
				{
					...
					if (remainingViewBoxHints.Any())
					{
						...
						for (var i = remainingViewBoxHints.Count(); i >= 0; i--)
						{
							...
							var results = ViewBoxOptimzer.Optimize(h, w, hintElements, varClone, timeout);
				...
				if (iteration > 0 && maxIterations < iteration || (unfitViewBoxes.Any() && diff.AreSame()))
				{
					...
					{
						anyFailures = true;
						Debug.WriteLine("Optimize failed:" + string.Join(",", unfitViewBoxes));

...
namespace RadialReview.Utilities
{
	public static class TimingUtility
	{
		...
		public static int STANDARD_SCORECARD_WEEKS = 17;
    ...
    public static double? ReviewDurationMinutes(List<AnswerModel> answers, TimeSpan excludeLongerThan)
		{
			var ordered = answers.ToListAlive().OrderBy(x => x.CompleteTime).GroupBy(x => x.CompleteTime).Select(x => new {
				time = x.FirstOrDefault().CompleteTime,
				count = x.Count()
			});
				...
				case DateOffset.MondayOfFourthWeek:
					{
						...
						if (s.DayOfWeek <= DayOfWeek.Monday)
							return s.AddDaysSafe(6.9999 * 4).StartOfWeek(DayOfWeek.Monday);
						return s.AddDaysSafe(6.9999 * 3).StartOfWeek(DayOfWeek.Monday);
					}
		...
		private static List<L10MeetingVM.WeekVM> GetWeeks(ITimeData settings, DateTime endDate, DateTime? highlightDate, bool includeNextWeek, bool useWeekstartForWeekNumber = false, DateRange range = null)
		{
			...
			var s = (serverTimeMeetingStart ?? endDate).StartOfWeek(DayOfWeek.Sunday).AddDaysSafe(-7 * TimingUtility.STANDARD_SCORECARD_WEEKS);
			...
			if (range != null)
			{
				...
				{
					e = range.EndTime.AddDaysSafe(7).StartOfWeek(DayOfWeek.Sunday);
				}
			}
			...
			{
				var currWeek = false;
				var next = s.AddDaysSafe(7);
				var s1 = s;
				var displayDate = s.AddDaysSafe(-7).AddDaysSafe(6.9999).StartOfWeek(weekStart);
				var displayDateNext = displayDate.AddDaysSafe(7);
		...
		{
			log.Debug("Called GetMonths with " + settings.WeekStart + "," + settings.TimezoneOffset);
			var weekNumber_StartOfWeek = DayOfWeek.Sunday;
			if (useWeekstartForWeekNumber)
				weekNumber_StartOfWeek = settings.WeekStart;
			var ed = (highlightDate ?? endDate);
			var sd = ed.AddMonths(-13);
			var e = new DateTime(ed.Year, ed.Month, 1).AddDaysSafe(6.9999).StartOfWeek(DayOfWeek.Sunday);
			var s = new DateTime(sd.Year, sd.Month, 1).AddDaysSafe(6.9999).StartOfWeek(DayOfWeek.Sunday);
			if (range != null)
			{
				s = new DateTime(range.StartTime.Year, range.StartTime.Month, 1).AddDaysSafe(6.9999).StartOfWeek(DayOfWeek.Sunday);
				e = new DateTime(range.EndTime.Year, range.EndTime.Month, 1).AddDaysSafe(6.9999).StartOfWeek(DayOfWeek.Sunday);
				if (e != range.EndTime)
				{
					e = e.AddMonths(1);
				}
			...
			while (true)
			{
				var currWeek = false;
				var n = s.AddMonths(1);
				var next = new DateTime(n.Year, n.Month, 1).AddDaysSafe(6.9999).StartOfWeek(DayOfWeek.Sunday);
				var s1 = s;
				if (highlightDate.NotNull(x => s1.AddDaysSafe(7.0).StartOfWeek(weekNumber_StartOfWeek) <= x.Value && x.Value < next.AddDays(7.0).StartOfWeek(weekNumber_StartOfWeek)))
					currWeek = true;
				var display = s.AddDays(6.9999).StartOfWeek(settings.WeekStart).AddMinutes(-settings.TimezoneOffset);
				weeks.Add(new L10MeetingVM.WeekVM() { DisplayDate = display, StartDate = display.AddMinutes(-settings.TimezoneOffset), ForWeek = s.StartOfWeek(DayOfWeek.Sunday), IsCurrentWeek = currWeek, });
				s = next;
				if (s > e)
					break;
			}
			...
			switch (scorecardPeriod)
			{
				...
				case ScorecardPeriod.Quarterly:
					...
					for (var i = 1; i <= 4; i++)
					{
						...
						if (w < end)
						{
							return new DateRange(yearStart.GetDate(year, i - 1).AddMinutes(-(timezoneOffset)), end);
						}
					}
    ...
    public static ScorecardPeriod GetPeriodByFrequency(Frequency frequency)
    {
      switch(frequency)
      {
        case Frequency.WEEKLY:
          return ScorecardPeriod.Weekly;
        case Frequency.MONTHLY:
          return ScorecardPeriod.Monthly;
        case Frequency.QUARTERLY:
          return ScorecardPeriod.Quarterly;
        default:
          throw new ArgumentOutOfRangeException("scorecardPeriod");
      }
    }
		...
		public static List<L10MeetingVM.WeekVM> GetPeriods(TimeSettings ts, DateTime endDate, DateTime? highlightDate, bool includeNextWeek, bool useWeekstartForWeekNumber = false, DateRange range = null)
		{
			...
			switch (settings.Period)
			{
				case ScorecardPeriod.Weekly:
					...
					break;
				case ScorecardPeriod.Monthly:
					output = GetMonths(settings, endDate, highlightDate, includeNextWeek, useWeekstartForWeekNumber, range);
					break;
				case ScorecardPeriod.Quarterly:
					output = GetQuarters(settings, endDate, highlightDate, includeNextWeek, useWeekstartForWeekNumber, range);
					break;
				default:
					throw new ArgumentOutOfRangeException("scorecardPeriod");
			}
    ...
    public static long GetDaysSinceEpoch(DateTime day)
    {
      var epoch = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
      var span = day.Subtract(epoch);

      return (long)Math.Floor(span.TotalDays);
    }
    ...
    public static DateTime GetDateByDaySinceEpoch(long days)
    {
      return new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc).AddDays(days).AddSeconds(1);
    }
		...
		{
      var peridoSum = PeriodsAgo(startTime, -1 * periods, scorecardPeriod);
      var startOfWeek = DayOfWeek.Sunday;

      return scorecardPeriod switch
      {
        ScorecardPeriod.Weekly => peridoSum.StartOfWeek(startOfWeek),
        ScorecardPeriod.Monthly => peridoSum.StartOfMonth(startOfWeek),
        ScorecardPeriod.Quarterly => peridoSum.FirstWeekOfQuarter().StartOfWeek(startOfWeek),
        _ => throw new ArgumentOutOfRangeException("scorecardPeriod")
      };
		}
		...
		private static IList<string> GetDateTimePatterns(CultureInfo culture)
		{
			...
			return new string[]{info.FullDateTimePattern, info.LongDatePattern, info.LongTimePattern, info.ShortDatePattern, info.ShortTimePattern, info.MonthDayPattern, info.ShortDatePattern + " " + info.LongTimePattern, info.ShortDatePattern + " " + info.ShortTimePattern, info.YearMonthPattern // Consider the sortable pattern, ISO-8601 etc
            };
		}
		...
		private static List<DateTime> _FixOrderedDates_Year(List<DateTime> dates)
		{
			...
			for (var i = dates.Count - 1; i >= 0; i--)
			{
				...
				while (true)
				{
					cur = new DateTime(dates[i].Year - subYears, dates[i].Month, dates[i].Day, dates[i].Hour, dates[i].Minute, dates[i].Second, dates[i].Millisecond);
					if (cur < last)
						break;
					subYears += 1;
				}
		...
		private static List<DateTime> _FixOrderedDates(List<String> dates, CultureInfo culture, out string format)
		{
			...
			{
				GuessPatterns(d, culture).Where(x => x != null).ToList().ForEach(x => histogram.Add(x, x));
			}
			...
			int interp;
			var fixedDates = InterpolateDates(found, out interp);
			return fixedDates;
		}
		...
		public static IEnumerable<DateTime> GetWeeksBetween(DateTime scorecardStart, DateTime scorecardEnd)
		{
			...
			if (e > DateTime.MinValue.AddDays(6.999))
				e = e.AddDaysSafe(6.999).StartOfWeek(DayOfWeek.Sunday);
			var i = s;
			while (i <= e)
			{
				yield return i;
    ...
    public static IEnumerable<DateTime> GetMonthsBetween(DateTime scorecardStard, DateTime scorecardEnd)
    {
      DateTime firstDateMonth = scorecardStard.AddDays(1 - scorecardStard.Day);

      while (firstDateMonth <= scorecardEnd)
      {
        DateTime primerDomingoDelMes = firstDateMonth.AddDays((DayOfWeek.Sunday + 7 - firstDateMonth.DayOfWeek) % 7);

        yield return primerDomingoDelMes;

        firstDateMonth = firstDateMonth.AddMonths(1);
      }
    }

    public static IEnumerable<DateTime> GetDaysBetween(DateTime scorecardStard, DateTime scorecardEnd)
    {
      DateTime midnightStartDay = scorecardStard.Date;
      DateTime midnightEndDay = scorecardEnd.Date;

      while (midnightStartDay <= scorecardEnd)
      {
        yield return midnightStartDay;

        midnightStartDay = midnightStartDay.AddDays(1);
      }
    }

    public static IEnumerable<DateTime> GetQuarterlyBetween(DateTime scorecardStard, DateTime scorecardEnd)
    {
      DateTime firstMonthDay = scorecardStard.AddDays(1 - scorecardStard.Day);

      while (firstMonthDay <= scorecardEnd)
      {
        int trimestre = (firstMonthDay.Month - 1) / 3 + 1;

        if ((firstMonthDay.Month - 1) % 3 == 0)
        {
          DateTime primerDomingoDelMes = firstMonthDay.AddDays((7 - (int)firstMonthDay.DayOfWeek + (int)DayOfWeek.Sunday) % 7);
    ...
    public static List<DateTime> InterpolateDates(List<DateTime?> dates, out int interpolated)
		...
		public static List<SelectListItem> GetPossibleTimes(int? selected)
		{
			...
			possibleTimes.Add(new SelectListItem() { Selected = (selected ?? -1) == -1, Text = "Do not send e-mail", Value = "-1" });
			for (int i = 0; i < 24; i++)
			{
				var name = " AM (GMT)";
				if (i == 0)
					...
					name = "" + (i - 12) + " PM (GMT)";
				possibleTimes.Add(new SelectListItem() { Selected = selected == i, Text = name, Value = "" + i });
			}
    ...
    public static bool IsPreviousYear(DateTime forWeek)
    {
      int currentYear = DateTime.Now.Year;
      return forWeek.Year < currentYear;
    }
  }
}